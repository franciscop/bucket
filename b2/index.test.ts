...
function withAuthMock(handler: FetchHandler = () => Promise.resolve(makeResponse(null))): FetchHandler {
  return (url, init) => {
...
async function makeBucket(handler?: FetchHandler): Promise<BackBlazeInstance> {
  ...
  const bucket = BackBlaze("test-bucket", { id: "test-id", secret: "test-key" });
...
const B2_LIST_RESPONSE = {
  files: [
    { fileName: "hello.txt", fileId: "id-hello", contentType: "text/plain", contentLength: 5, uploadTimestamp: 1704067200000 },
    { fileName: "data/world.json", fileId: "id-world", contentType: "application/json", contentLength: 25, uploadTimestamp: 1704153600000 },
  ],
  nextFileName: null,
};
...
describe("B2 bucket.list()", () => {
  ...
  it("populates metadata (type, size, date, url)", async () => {
    ...
    expect(files[0].url).toBe("https://f001.backblazeb2.com/file/test-bucket/hello.txt");
  });

  it("handles empty bucket", async () => {
    const bucket = await makeBucket((url) => {
      if ((url as string).includes("b2_list_file_names"))
        return Promise.resolve(makeResponse(JSON.stringify({ files: [], nextFileName: null })));
      return Promise.resolve(makeResponse(null));
    });
    const files = await bucket.list();
  ...
  it("follows pagination via nextFileName", async () => {
    ...
    const bucket = await makeBucket((url) => {
      if ((url as string).includes("b2_list_file_names")) {
        listCalls++;
        const page = listCalls === 1
          ? { files: [{ fileName: "a.txt", fileId: "id-a", contentType: "text/plain", contentLength: 1, uploadTimestamp: 0 }], nextFileName: "b.txt" }
          : { files: [{ fileName: "b.txt", fileId: "id-b", contentType: "text/plain", contentLength: 1, uploadTimestamp: 0 }], nextFileName: null };
...
describe("B2 file().write()", () => {
  ...
  it("requests upload URL then POSTs the file", async () => {
    ...
    const bucket = await makeBucket((url, init) => {
      ...
      if ((url as string).includes("/upload")) {
        return Promise.resolve(makeResponse(JSON.stringify({ fileId: "new-file-id" })));
      }
      return Promise.resolve(makeResponse(null));
    });

    await bucket.file("hello.txt").write("hello world");
    expect(requests.some((r) => r.url.includes("b2_get_upload_url"))).toBe(true);
  ...
  it("sends the correct B2 upload headers", async () => {
    ...
    const bucket = await makeBucket((url, init) => {
      ...
      if ((url as string).includes("/upload")) {
        uploadHeaders = Object.fromEntries(new Headers(init?.headers).entries());
        return Promise.resolve(makeResponse(JSON.stringify({ fileId: "new-id" })));
      }
      return Promise.resolve(makeResponse(null));
    });
...
describe("B2 file().publicUrl()", () => {
  ...
  it("returns a URL for files returned from list()", async () => {
    ...
    const files = await bucket.list();
    expect(files[0].publicUrl()).toBe("https://f001.backblazeb2.com/file/test-bucket/hello.txt");
  });
});
  ...
  it("returns a URL with authorization token", async () => {
    const bucket = await makeBucket((url) => {
      if ((url as string).includes("b2_get_download_authorization")) {
        return Promise.resolve(makeResponse(JSON.stringify({ authorizationToken: "signed-token-abc" })));
      }
      return Promise.resolve(makeResponse(null));
    });
  ...
  it("accepts string duration", async () => {
    ...
    const bucket = await makeBucket((url, init) => {
      if ((url as string).includes("b2_get_download_authorization")) {
        ...
        return Promise.resolve(makeResponse(JSON.stringify({ authorizationToken: "tok" })));
      }
      return Promise.resolve(makeResponse(null));
    });
...
describe("B2 file().uploadUrl()", () => {
  ...
  it("returns null (B2 does not support presigned uploads)", async () => {
    const bucket = await makeBucket();
    expect(await bucket.file("hello.txt").uploadUrl({ expires: 3600 })).toBeNull();
  });
});
...
describe("B2 bucket.remove()", () => {
  ...
  it("deletes all listed files and returns them", async () => {
    ...
    const bucket = await makeBucket((url, init) => {
      if ((url as string).includes("b2_list_file_names")) {
        listCallCount++;
        // After first list (for remove), subsequent calls (from exists check) return empty
        const response = listCallCount === 1 ? B2_LIST_RESPONSE : { files: [], nextFileName: null };
        return Promise.resolve(makeResponse(JSON.stringify(response)));
      }
      if ((url as string).includes("b2_delete_file_version")) {
        deleteRequests.push(url as string);
        return Promise.resolve(makeResponse(JSON.stringify({ fileId: "deleted", fileName: "deleted" })));
      }
      return Promise.resolve(makeResponse(null));
    });
  ...
  it("copies then deletes the original", async () => {
    ...
    const bucket = await makeBucket((url, init) => {
      ...
      if ((url as string).includes("/upload"))
        return Promise.resolve(makeResponse(JSON.stringify({ fileId: "new-id" })));
      if ((url as string).includes("b2_list_file_names")) {
        // First call: info() needs to find the file to get its id
        // Subsequent calls: exists() check returns empty (file deleted)
        const response = listCalls++ === 0
          ? { files: [{ fileName: "src.txt", fileId: "src-id", contentType: "text/plain", contentLength: 7, uploadTimestamp: 0 }], nextFileName: null }
          : { files: [], nextFileName: null };
        return Promise.resolve(makeResponse(JSON.stringify(response)));
      }
      if ((url as string).includes("b2_delete_file_version"))
        return Promise.resolve(makeResponse(JSON.stringify({ fileId: "src-id", fileName: "src.txt" })));
      return Promise.resolve(makeResponse(null));
    });
    await bucket.file("src.txt").moveTo("dst.txt");
    expect(requests.some((r) => r.includes("/upload"))).toBe(true);
    expect(requests.some((r) => r.includes("b2_delete_file_version"))).toBe(true);
  });
});
